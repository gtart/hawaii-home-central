        ...
        <div className="max-w-3xl mx-auto">
          ...
          {!loading && items.length === 0 && (
            <div className="text-center py-12">
              <svg className="w-10 h-10 text-cream/30 mx-auto mb-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
              ...
              <p className="text-sm text-cream/45">No unsorted items</p>
          ...
          {nextCursor && !loading && (
            <div className="flex justify-center mt-6">
              <button
                ...
                className="px-4 py-2 text-sm text-cream/55 hover:text-cream/70 bg-stone-200 rounded-lg transition-colors disabled:opacity-50"
              >
...
function InboxItemCard({
  ...
  return (
    <div className="flex items-start gap-3 p-3 bg-stone-50 border border-cream/12 rounded-lg group">
      {/* Thumbnail */}
      {(item.thumbnailUrl || item.imageUrl) ? (
        ...
        <div className="w-14 h-14 rounded-lg bg-stone-200 flex items-center justify-center shrink-0">
          {item.type === 'NOTE' ? (
            <svg className="w-6 h-6 text-cream/35" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1">
          <span className="text-[10px] text-cream/40 uppercase tracking-wide">{typeLabel}</span>
          {hostname && (
            <>
              <span className="text-cream/30">·</span>
              <span className="text-[11px] text-cream/40 truncate">{hostname}</span>
            </>
          )}
          <span className="text-cream/30">·</span>
          <span className="text-[11px] text-cream/40">{relativeTime(item.createdAt)}</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        <button
          type="button"
          onClick={onDismiss}
          className="p-1.5 text-cream/35 hover:text-cream/65 transition-colors"
          title="Dismiss"
        >
          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
