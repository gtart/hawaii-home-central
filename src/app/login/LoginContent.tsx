...
export function LoginContent() {
  ...
  useEffect(() => {
    if (session?.user && !error) {
      ...
      // If still here after 3 seconds, show a manual link
      const timeout = setTimeout(() => setRedirectFailed(true), 3000)
      return () => clearTimeout(timeout)
    }
  }, [session, callbackUrl, error])

  // Show error even if session exists (e.g. callback failed but stale session cookie)
  if (error) {
    return (
        ...
        <div className="max-w-sm mx-auto text-center">
          {redirectFailed ? (
            <>
              ...
              >
                Continue to {callbackUrl === '/app' ? 'your tools' : callbackUrl} →
              </Link>
            </>
          ) : (
