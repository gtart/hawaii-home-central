...
export async function POST(request: Request) {
  ...
  const access = await resolveToolAccess(userId, projectId, 'project_alignment')
  ...
  if (file.size === 0) return NextResponse.json({ error: 'Empty file received — please try again.' }, { status: 400 })
  ...
  const pathname = `alignment/${projectId}/${Date.now()}-${file.name}`
  ...
  try {
    ...
    const id = `alph_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`
    ...
    try {
      ...
      const thumbPathname = `alignment/${projectId}/thumb_${Date.now()}-${file.name.replace(/\.[^.]+$/, '.jpg')}`
