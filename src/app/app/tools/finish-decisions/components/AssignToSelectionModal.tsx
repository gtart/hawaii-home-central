...
export function AssignToSelectionModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative bg-stone border-t sm:border border-cream/15 rounded-t-xl sm:rounded-xl w-full sm:max-w-md max-h-[80vh] overflow-hidden flex flex-col">
        {/* Header */}
        <div className="border-b border-cream/15 px-5 py-4 flex items-center justify-between shrink-0">
          <div>
        ...
        {selections.length > 5 && (
          <div className="px-5 pt-3">
            <input
              ...
              autoFocus
              className="w-full px-3 py-2 bg-basalt border border-cream/15 text-cream text-sm rounded-lg placeholder:text-cream/40 focus:outline-none focus:border-sandstone/50"
            />
          </div>
        )}
            ...
            <div className="space-y-2">
              {filtered.map((decision) => (
                <button
                  ...
                  className="w-full text-left px-4 py-3 bg-basalt rounded-lg border border-cream/12 hover:border-cream/25 hover:bg-basalt/50 transition-all"
                >
